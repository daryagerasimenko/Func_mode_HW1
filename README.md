...
Мы видим, что график монотонно возрастает. От выбранных русскоязычных экпрессивов ожидается такой же монотонный график, но убывающий. 
...
На основе той же модели, были построены новые графики регрессионных моделей уже по данным второго корпуса. 
...
_Жаль, что к ним не присоединиться Шон из Токио._
...
в отличие от отзывов с "Кинопоиска", довольно формальный. 

Сама модель (кроме тонкостей, связанных с тем, что здесь немного другой формат корпуса) никак не меняется. Соответственно, графики регрессионной модели для тех же экспрессивов, но в контекстах из новостного корпуса, выглядят следующим образом:
